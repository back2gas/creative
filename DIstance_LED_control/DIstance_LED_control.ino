...
void loop() {
  ...
  else if(_DIST_MIN <distance && distance <= 200) {    // In desired Range
        analogWrite(PIN_LED, 2.5*(distance - 100));
  }
  else if(200 <distance && distance <_DIST_MAX) {    // In desired Range
        analogWrite(PIN_LED, 250 - 2.5*(distance - 200));
  }
